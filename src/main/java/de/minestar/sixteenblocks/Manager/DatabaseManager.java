...
public class DatabaseManager extends AbstractDatabaseHandler {
    ...
    protected DatabaseConnection createConnection(File dataFolder) throws Exception {
        ...
        if (!configFile.exists()) {
            DatabaseUtils.createDatabaseConfig(DatabaseUtils.TYPE_MYSQL, configFile, Core.getInstance().getDescription().getName());
            return null;
        }

        config.load(configFile);
        return new DatabaseConnection(config.getString("Host"), config.getString("Port"), config.getString("Database"), config.getString("User"), config.getString("Password"));

    }

    @Override
    protected void createStatements(Connection connection) throws Exception {
    ...
    @Override
    protected void createStructure(Connection connection) throws Exception {
